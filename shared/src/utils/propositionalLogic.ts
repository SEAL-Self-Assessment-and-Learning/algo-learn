...
function computeNewTerms(
  ...
  let index1 = 0
  while (intersection1 > 0) {
    if (intersection1 & 1) {
      const newDictTerm = {
        0: (disjunctionDict[i][0] | disjunctionDict[j][0]) ^ (2 ** index1),
        1: (disjunctionDict[i][1] | disjunctionDict[j][1]) ^ (2 ** index1),
      }
 ...
 *
 *
 * Note: Only reduction, not using "weakening"
 *
 * @param disjunctionTerms
 * @param rounds
 */
export function getResolutionTerms(disjunctionTerms: Literal[][], rounds: number = 10): Literal[][] {
  const { varNamesIDs, varNamesIDsInverted } = getVarNamesIds(disjunctionTerms)
  const disjunctionDict = createDisjunctionDict(disjunctionTerms, varNamesIDs)
  ...
  let lastDtlsLength = 0
  for (let _ = 0; _ < rounds; _++) {
    const extendDTL: { [key: number]: number }[] = []
    for (let i = 0; i < disjunctionDict.length; i++) {
      for (let j = max([i + 1, lastDtlsLength]); j < disjunctionDict.length; j++) {
        computeNewTerms(
          disjunctionDict[i][0] & disjunctionDict[j][1],
          disjunctionDict,
          i,
          j,
          uniqueTerms,
        ...
        computeNewTerms(
          disjunctionDict[i][1] & disjunctionDict[j][0],
          disjunctionDict,
          i,
          j,
          uniqueTerms,
    ...
    lastDtlsLength = disjunctionDict.length - 1
    for (const extend of extendDTL) {
      disjunctionDict.push(extend)
    }
  }
...
function createLiteralFromDict(
  disjunctionDict: { [key: number]: number }[],
  varNamesIds: { [key: number]: string },
): Literal[][] {
  const literalList: Literal[][] = []
  for (const disjunction of disjunctionDict) {
    const newLiterals: Literal[] = []
    for (const variable of getOnePositions(disjunction[0])) {
      newLiterals.push(new Literal(varNamesIds[variable], true))
    }
    for (const variable of getOnePositions(disjunction[1])) {
      newLiterals.push(new Literal(varNamesIds[variable], false))
    }
    literalList.push(newLiterals)
  }
  return literalList
}
...
function createDisjunctionDict(
  ...
  const disjunctionDict: { [key: number]: number }[] = []
  for (const literals of literalList) {
    const termDict: { [key: number]: number } = { 0: 0, 1: 0 }
