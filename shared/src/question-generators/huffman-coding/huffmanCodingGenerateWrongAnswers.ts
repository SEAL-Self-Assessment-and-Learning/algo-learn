...
import {
    createHuffmanCoding, createHuffmanCodingBitError,
...
export function generateWrongAnswerSwitchLetters (random: Random, currentTree: TreeNode, word: string) {
    ...
    const [randomKey1, randomKey2] = keySet.sort(() => random.uniform() - 0.5).slice(0, 2);
    [huffmanDict[randomKey1], huffmanDict[randomKey2]] = [huffmanDict[randomKey2], huffmanDict[randomKey1]];
    const resultWord: string = word.split('').map(char => huffmanDict[char]).join('');
 ...
 * @param currentTree
 */
export function generateWrongAnswerFalseTreeConstrution(random: Random,
                                                        word: string,
...
export function generateWrongAnswerReduceCodeOfLetter(word: string,
    ...
    for (const current_key in huffmanDict) {
        ...
        if (huffmanDict[current_key].length > huffmanDict[current_longest_key].length) {
            current_longest_key = current_key
        }
    }
