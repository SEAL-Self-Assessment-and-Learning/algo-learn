<script lang="ts">
  ...
  function resizeObserverAction(node: HTMLElement) {
...
<!-- Main container -->
<div {...rest} class="p-6 sm:p-1 md:p-4" use:resizeObserverAction>
  <!-- Filter + Sort Header -->
    ...
    <div class="mb-6 grid grid-cols-1 gap-6 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
      {#each row as g (g.slug)}
        <button
          ...
          class="group flex cursor-pointer flex-col justify-between rounded-xl border p-4 text-center shadow-sm transition-all duration-200 hover:border-blue-500 hover:shadow-lg
        ...
        >
          <div>
            ...
            <h2 class="mb-2 text-lg font-bold text-gray-800">{g.name[lang]}</h2>
